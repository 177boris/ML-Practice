...
class OCRNeuralNetwork:
    ...
    def __init__(self, num_hidden_nodes, data_matrix, data_labels, train_indices, use_file = False):
    ...
    # if (not os.path.isfile(OCRNeuralNetwork.NN_FILE_PATH) or not use_file):
        if (not os.path.isfile(OCRNeuralNetwork.NN_FILE_PATH) or not use_file):
            ...
            self.train([TrainData(self.data_matrix[i], int(self.data_labels[i])) for i in train_indices])
            self.save()
